...
import org.deadmaze.database.embeds.TribeHistoricEntry;
import org.deadmaze.database.embeds.TribeRank;
import org.deadmaze.packets.TribullePacket;
import org.deadmaze.packets.identifiers.TribulleNew;
import org.deadmaze.utils.Langue;
...
public final class ParseTribulle {
    ...
    private void sendCreateNewGlobalChannel(final long tribulleId, final String chatName) {
        if(!chatName.matches("^[a-zA-Z0-9_.-]*$")) {
        ...
        if (this.server.getChats().get(chatName).size() > 100) {
    ...
    private void sendCreateNewTribe(final long tribulleId, final String tribeName) {
        ...
        if(this.client.getAccount().getReputationPoints() < 500) {
            this.sendTribullePacket(TribulleNew.Send.ET_ResultatDemandeCreerTribu, new ByteArray().writeInt(tribulleId).writeByte(14));
            return;
        }

        this.client.getAccount().setReputationPoints(this.client.getAccount().getReputationPoints() - 100);
