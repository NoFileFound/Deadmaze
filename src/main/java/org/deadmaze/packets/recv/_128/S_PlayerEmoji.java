...
// Packets
import org.deadmaze.packets.send._112.C_PlayerEmoji;
...
public final class S_PlayerEmoji implements RecvPacket {
    ...
    public void handle(Client client, int fingerPrint, ByteArray data) {
        if(client.sceneLoadingInfo != SceneLoading.LOADED) {
            Application.getLogger().debug(Application.getTranslationManager().get("abnormalactivity", client.getPlayerName(), "S_PlayerEmoji"));
            client.getServer().getTempBlackList().add(client.getIpAddress());
            client.closeConnection();
            return;
        }

        client.getRoom().sendAll(new C_PlayerEmoji(client.getSessionId(), data.readByte()));
    }
